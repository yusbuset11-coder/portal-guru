...
st.markdown(
    ...
    <div style="background: linear-gradient(135deg, #1e293b 0%, #111827 100%); padding: 25px 30px; border-radius: 14px; border: 1px solid #334155; margin-bottom: 25px;">
        <h2 style="color: #ffffff; margin: 0 0 10px 0; font-size: 26px;">🎯 Digitalisasi Presensi Siswa</h2>
...
if menu == "📝 Pencatatan Presensi Harian":
  ...
  if df_siswa.empty or "Sekolah" not in df_siswa.columns:
    ...
    st.markdown("---")
    st.subheader(f"Daftar Kehadiran Siswa - {selected_sekolah} ({selected_kelas})")
        ...
        if submitted:
          if not mapel:
            ...
            if ws_absensi and rows_to_add:
              ...
              st.balloons()
              st.success(
                  "🎉 Data presensi berhasil disimpan ke Google Spreadsheet Anda!"
              )
            else:
        ...
        with col_btn1:
          if st.button(
            ...
            if ws_rekap:
              ...
              st.balloons()
              st.success(
                  f"✅ Rekap Semester {semester_type} berhasil disinkronkan ke"
                  f" tab '{sheet_target}' di Spreadsheet!"
              )

        with col_btn2:
          # --- Tombol Download Excel dengan Auto-Fit Kolom & index=False ---
          output_presensi = BytesIO()
          with pd.ExcelWriter(output_presensi, engine="openpyxl") as writer:
            df_rekap.to_excel(
                writer,
                index=False,
                sheet_name=f"Rekap_Semester_{semester_type}",
            )

            # Otomatis merapikan lebar kolom di Excel
            worksheet = writer.sheets[f"Rekap_Semester_{semester_type}"]
          ...
          output_presensi.seek(0)

          st.download_button(
              label=f"📥 Download Rekap {semester_type} (.xlsx)",
              data=output_presensi,
              file_name=f"Rekap_Presensi_Semester_{semester_type}_{sel_sek}_{sel_kls}.xlsx".replace(
                  " ", "_"
              ),
              mime=(
              ...
              use_container_width=True,
              key=f"btn_dl_presensi_{semester_type}",
          )
