...
def generate_professional_word_document(
  ...
  metadata = [
      ...
      (
          ...
          f"{jenis_asesmen} ({sub_asesmen}) - {jenis_soal}",
      ),
  ]
...
with tab_beranda:
  ...
  with st.container(border=True):
    ...
    st.markdown(
        "* **Kunci Jawaban & Pembahasan:** Dilengkapi opsi pembahasan mendalam"
        " untuk setiap butir soal."
    )
    st.markdown(
        "* **Penyimpanan Cloud & Word Profesional:** Simpan ringkasan asesmen ke"
...
with tab_generator:
  ...
  with col2:
    materi = st.text_input(
        "Materi / Topik",
        placeholder="Contoh: Mengidentifikasi Makna Kata",
        key="input_materi_gen",
    )
  ...
  with col7:
    ...
    sub_asesmen = st.selectbox(
        "Bentuk / Sub Jenis Asesmen", sub_asesmen_options, key="gen_sub_jenis"
    )

  col8, col9, col10 = st.columns(3)
  with col8:
    jumlah_soal = st.number_input(
        "Jumlah Butir Soal", min_value=1, max_value=20, value=5, key="gen_jml_soal"
    )
  with col9:
  ...
  if st.button(
      "✨ Buat Instrumen Asesmen dengan Gemini AI 🚀",
  ...
  if "generated_soal" in st.session_state and st.session_state.generated_soal:
    ...
    with col_dl2:
      if st.button(
          "💾 Simpan ke Bank Soal Spreadsheet",
          use_container_width=True,
          key="btn_simpan_bank",
      ):
            ...
            try:
              ...
              try:
                ...
                ws_bank = ss_user.add_worksheet(
                    title="Bank Soal SAKTI", rows="1000", cols="10"
                )
              ...
              st.success(
                  "🎉 Metadata soal berhasil disimpan ke tab 'Bank Soal SAKTI' di"
                  " spreadsheet Anda!"
              )
            except Exception as e:
...
with tab_bank:
  ...
  st.write(
      "Daftar arsip ringkasan asesmen yang pernah Anda buat dan simpan. Klik"
      " tombol **Unduh Soal** pada kolom aksi untuk men-generate dan mengunduh"
        ...
        for idx, row in df_bank.iterrows():
          ...
          with row_cols[5]:
            ...
            if st.button("📥 Unduh Soal", key=btn_key, use_container_width=True):
              with st.spinner(
                  "⏳ Menyiapkan dokumen Word dari arsip..."
              ):
                try:
                  if jenjang == "SD":
                    aturan_opsi = "3 opsi (A sampai C)"
                  ...
                  st.session_state[f"file_bytes_{idx}"] = (
                      docx_file.getvalue()
                  )
                  st.session_state[f"file_name_{idx}"] = (
                      f"Asesmen_{mapel}_{materi}.docx"
                  )
...
with tab_rekap:
  ...
  with col_r1:
    tanggal_input = st.date_input(
        "Pilih Tanggal", value=datetime.now(), key="rekap_tgl"
    )
    ...
    r_sekolah = st.selectbox(
        "Pilih Sekolah", daftar_sekolah, key="input_sekolah_rekap"
    )
  ...
  with col_r2:
    ...
    for idx, r in enumerate(siswa_filtered, start=1):
      try:
        absen_raw = r.get(
            "No_Absen", r.get("No Absen", r.get("No", r.get("Nomor", idx)))
        )
    ...
    r_no_absen = st.selectbox(
        "Pilih No. Absen Siswa", list_absen, key="rekap_absen"
    )
    ...
    r_nilai = st.number_input(
        "Nilai Siswa", min_value=0, max_value=100, value=80, key="rekap_nilai_num"
    )
    ...
    r_sub_jenis = st.selectbox(
        ...
        sub_asesmen_input_options,
        key="input_sub_jenis_rekap",
    )

  if st.button(
      "💾 Simpan Nilai ke Google Sheets",
      use_container_width=True,
      key="btn_simpan_rekap",
  ):
    if not r_mapel or not r_nama_siswa:
      st.warning(
          "⚠️ Mohon lengkapi Mata Pelajaran dan pastikan Nama Siswa terpilih."
      )
    else:
      try:
  ...
  if not df_rekap.empty:
    ...
    if not df_final_dl.empty:
      ...
      st.download_button(
          ...
          data=output_excel,
          file_name=f"Rek_Nilai_{dl_sekolah}_{dl_kelas}_{dl_mapel}.xlsx".replace(
              " ", "_"
          ),
          mime=(
