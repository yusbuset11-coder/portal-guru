...
if menu == "🏠 Beranda Asesmen":
  st.write(
      "Gunakan kecerdasan buatan untuk merancang soal asesmen formatif, sumatif,"
      " kisi-kisi, serta rubrik penilaian secara cepat dan akurat."
  )
            ...
            try:
              ...
              try:
                ...
                ws_bank = ss_user.add_worksheet(
                    title="Bank Soal SAKTI", rows="1000", cols="11"
                )
              ...
              st.success(
                  "🎉 Soal beserta metadata berhasil disimpan ke tab 'Bank Soal"
                  " SAKTI' di spreadsheet Anda!"
              )
            except Exception as e:
...
# --- MENU 3: BANK SOAL & ASESMEN TERSIMPAN ---
  ...
  st.write(
      "Daftar arsip ringkasan asesmen yang pernah Anda buat dan simpan. Tombol"
      " untuk mengunduh dokumen Word (.docx) tersedia langsung di kolom paling"
      " kanan."
  )
    ...
    with st.spinner("Memuat data bank soal..."):
      ...
      if df_bank.empty or len(df_bank) == 0:
        ...
        for idx, row in df_bank.iterrows():
          ...
          jml_soal_val = (
              int(row.get("Jumlah_Soal", 5))
              if str(row.get("Jumlah_Soal", 5)).isdigit()
              else 5
          )
          ...
          with row_cols[5]:
            if konten_val:
              ...
              st.download_button(
                  label="📥 Unduh Word",
                  data=docx_file_bank,
                  file_name=f"Asesmen_{mapel}_{materi}.docx".replace(
                      " ", "_"
                  ),
                  mime=(
                      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
...
# --- MENU 4: INPUT DAN REKAP NILAI SISWA (TATA LETAK DISEJAJARKAN) ---
  ...
  with col_r1:
    ...
    kelas_filtered = [
        r for r in master_data if str(r.get("Sekolah", "")).strip() == r_sekolah
    ]
    daftar_kelas = []
