...
# Sidebar Profil (Tanpa Menu Navigasi Samping)
with st.sidebar:
  st.markdown(
      f"""
      <div class="user-profile-box">
          <span style="font-size: 24px;">👨‍💻</span><br>
          <b style="color: #facc15; font-size: 14px;">{st.session_state.get('guru_nama', '')}</b><br>
...
if not spreadsheet_id:
  st.warning(
      "⚠️ Spreadsheet ID belum dikonfigurasi. Silakan kembali ke Halaman Utama"
      " untuk mengatur konfigurasi terlebih dahulu."
  )
  st.stop()
...
if sh_guru is None:
  ...
  st.markdown(
      f"""
      <div style="background: linear-gradient(135deg, #1e293b 0%, #111827 100%); padding: 25px 30px; border-radius: 14px; border: 1px solid #334155; margin-bottom: 20px;">
  ...
  # NAVIGASI UTAMA (Berada di Halaman Utama & Berderet Horizontal)
  menu_jurnal = st.radio(
      "**Pilih Menu Jurnal**",
      [
          "🏠 Beranda Jurnal",
    ...
    with st.container(border=True):
      with st.form("form_jurnal_mengajar"):
        ...
        catatan_kejadian = st.text_area(
            ...
            placeholder="Catatan penting siswa atau kendala selama KBM...",
        )
        ...
        if btn_simpan_jurnal:
          if not mata_pelajaran or not kompetensi_dasar:
            st.error(
                "⚠️ Mata Pelajaran dan Materi/Tujuan Pembelajaran wajib diisi!"
            )
          else:
            with st.spinner("Menyimpan jurnal ke Google Sheets..."):
              try:
                ...
                ws_jurnal = sh_guru.add_worksheet(
                    title="Jurnal Mengajar", rows="1000", cols="7"
                )

              # Pastikan header bersih ada di baris 1
              rows_check = ws_jurnal.get_all_values()
              if not rows_check or rows_check[0] != EXPECTED_JURNAL_HEADER:
                ws_jurnal.insert_row(EXPECTED_JURNAL_HEADER, index=1)
