...
def load_master_registry():
  try:
    ...
    st.error(
        f"❌ Gagal terhubung ke Google Spreadsheet Master Registry. Detail Error:"
        f" {e}"
    )
    return None
...
st.markdown(
    ...
    <div class="main-header-card">
        ...
        <div class="main-title-text" style="color: #ffffff !important; font-size: 30px; font-weight: bold;">Portal Administrasi Siswa Terintegrasi</div>
    ...
    if btn_login:
      if not user_input:
        ...
        with st.spinner("Memeriksa kredensial ke Master Registry..."):
          client = get_gspread_client()
          try:
            sh_master = client.open_by_key(MASTER_REGISTRY_ID)
            ws_master = sh_master.worksheet("DATABASE_MASTER_REGISTRY")
            data_registry = ws_master.get_all_records()
          except Exception as e:
            data_registry = None
            st.error(f"Gagal memuat master registry: {e}")
        ...
        if data_registry:
          ...
          if not matched.empty:
            ...
            if status_akun != "AKTIF":
              st.error(
                  "❌ Maaf, akun Anda berstatus TIDAK AKTIF. Silakan hubungi"
                  " administrator."
              )
            else:
              ...
              # --- OTOMATISASI CATAT WAKTU LAST_LOGIN ---
              try:
                # Cari nomor baris (gspread menghitung baris mulai dari 1, ditambah 1 karena header di baris 1)
                matched_row_idx = matched.index[0] + 2
                header_row = ws_master.row_values(1)

                if "Last_Login" in header_row:
                  col_idx = header_row.index("Last_Login") + 1
                  waktu_sekarang = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                  ws_master.update_cell(matched_row_idx, col_idx, waktu_sekarang)
              except Exception as err:
                print(f"Gagal memperbarui Last_Login: {err}")
              # ------------------------------------------
